...
{
    public static ValueTask<IValueResult<IEnumerable<double>>?> Example(DateTimeOffset first, DateTimeOffset last)
    {
    ...
    where TIndicator : IIndicator<TParameters, TInput, TOutput>
{
    ...
    {
        return HistoricalIndicatorExecutorX<TIndicator, TParameters, TInput, TOutput>.TryGetReverseOutput(ServiceProvider, range, Indicator, Parameters);
    }
}
