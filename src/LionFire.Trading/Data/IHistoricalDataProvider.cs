...
public static class IHistoricalDataProviderX
{
    public static async Task<IEnumerable<DataLoadResult>> GetData(this IHistoricalDataProvider provider, MarketSeriesBase series, DateTimeOffset? startDate, DateTimeOffset endDate, bool cacheOnly = false, bool writeCache = true, TimeSpan? maxOutOfDate = null, int totalDesiredBars = 0, bool forceRetrieve = false, bool forceReretrieveEmptyData = false, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            DateTimeOffset nextChunkDate;
            if (series.HasAccount && series.Account.IsBacktesting)
            {
                nextChunkDate = DateTimeOffsetX.ToDateTimeOffset(Math.Min(series.Account.BacktestEndDate.ToBinary(), series.OpenTime.First().ToBinaryWithoutOffset()));
            }
            else
            {
                if (cacheOnly)
                ...
                {
                    nextChunkDate = DateTimeOffset.UtcNow + TimeSpan.FromMinutes(1);
                }
            }
            ...
            do
            {
                if (nextChunkDate == default) break;
            ...
            ((nextChunkDate != default(DateTimeOffset) && nextChunkDate > startDate.Value)
    ...
    /// <returns></returns>
    public static async Task<DataLoadResult> GetDataForChunk(this IHistoricalDataProvider provider, MarketSeriesBase series, DateTimeOffset chunkStart, DateTimeOffset chunkEnd, bool cacheOnly = false, bool writeCache = true, TimeSpan? maxOutOfDate = null, bool forceRetrieve = false, bool forceReretrieveEmptyData = false, CancellationToken cancellationToken = default)
    {
        ...
        if (chunkStart >= series.DataStartDate && chunkEnd <= series.DataEndDate)
        {
            ...
            var nowPastLast = DateTimeOffset.UtcNow - series.OpenTime.LastValue;
        ...
        if (!forceRetrieve && (!forceReretrieveEmptyData || !cacheFile.EmptyData) && cacheFile.IsPersisted && (isUpToDateAfterClose || cacheFile.OutOfDateTimeSpan < maxOutOfDate.Value)
            //&& (chunkEnd > DateTimeOffset.UtcNow || !result.IsPartial)
            )
        {
