...
public abstract class SimulatedAccountBase_Old<TTemplate, TAccount> : AccountBase_Old<TTemplate>, ISimulatedAccount_Old, IInitializable
    where TTemplate : TSimulatedAccountBase
    where TAccount : IFeed_Old
{
    ...
    public IAccount_Old Account { get { return Accounts?.FirstOrDefault(); } }

    public List<IAccount_Old> Accounts { get; set; } = new List<IAccount_Old>();
    ...
    public SimulatedAccountBase_Old(ILogger logger, ILoggerFactory loggerFactory) : base(logger, loggerFactory)
    {
